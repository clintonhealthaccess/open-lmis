...
public class StockCardController extends BaseController {
  ...
  public ResponseEntity processStock(@PathVariable long facilityId,
    ...
    if (null == events || 0 >= events.size())
      return OpenLmisResponse.success(messageService.message("success.stock.event.none"));
    ...
    for (StockEvent event : events) {
      ...
      switch (event.getType()) {
        case ISSUE:
          entryType = StockCardEntryType.DEBIT;
          break;
        case RECEIPT:
          entryType = StockCardEntryType.CREDIT;
          break;
        case ADJUSTMENT:
          entryType = StockCardEntryType.ADJUSTMENT;
          break;
        default:
          break;
      }
